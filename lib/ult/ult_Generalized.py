...
image_folder_head = '/Disk2/hongwei/'
verb_trans = np.load(os.path.abspath(os.path.join(config.cfg.ROOT_DIR, 'ult', 'verb_trans.npy')))

def Get_Next_Instance_HO_HICO_DET_for_10w(Trainval_GT, Trainval_Neg, image_id, Pos_augment, Neg_select):
    im_orig, Human_augmented, _, _, Part_bbox, _, action_HO, action_PVP0, action_PVP1, action_PVP2, action_PVP3, action_PVP4, action_PVP5, num_pos, gt_10v = Augmented_HO_Neg_HICO_DET_for_10w(image_id, Trainval_GT, Trainval_Neg, Pos_augment, Neg_select)
    ...
    blobs['gt_verb'] = np.matmul(action_HO, verb_trans)
    return blobs

def Augmented_HO_Neg_HICO_DET_for_10w(image_id, trainval_GT, Trainval_Neg, Pos_augment, Neg_select):
    dataset = trainval_GT[image_id][0][4]
    image_folder = image_folder_head + dataset
    im_file = image_folder + image_id
    # im_file = image_folder_list[dataset] + image_id
    ...
    for i in range(Pos_augment):
        ...
        action_PVP0.append(Generate_action_PVP(GT[i][9]['pvp76_foot'], 12))
        action_PVP1.append(Generate_action_PVP(GT[i][9]['pvp76_leg'], 10))
        action_PVP2.append(Generate_action_PVP(GT[i][9]['pvp76_hip'], 5))
        action_PVP3.append(Generate_action_PVP(GT[i][9]['pvp76_hand'], 31))
        action_PVP4.append(Generate_action_PVP(GT[i][9]['pvp76_arm'], 5))
        action_PVP5.append(Generate_action_PVP(GT[i][9]['pvp76_head'], 13))
        action_hp10.append(Generate_action_PVP(GT[i][9]['hp10_list'], 10))
    ...
    return (
     im_orig, Human_augmented, None, None, part_bbox, None, action_HO, action_PVP0, action_PVP1, action_PVP2, action_PVP3, action_PVP4, action_PVP5, num_pos, action_hp10)
