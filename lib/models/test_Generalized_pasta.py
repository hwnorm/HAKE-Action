...
def im_detect(sess, net, im_file, Test_RCNN, keys, scores_P, scores_A, scores_L, bboxes, hdet, odet):
    This_image   = []
    ...
    # for Human_out in Test_RCNN[image_id]:
        ...
        if Human_out[2] == 1:
            ...
            if blobs['H_num'] > 0:
                ...
                P, L = net.test_image_H(sess, im_orig, blobs)
...
def test_net(sess, net, Test_RCNN, output_dir):
    ...
    keys, scores_P, scores_L, bboxes, hdet = [], [], [], [], []
    for i in range(80):
        keys.append([])
        scores_P.append([])
        # scores_A.append([])
    ...
    for line in glob.iglob(cfg.DATA_DIR + '/' + 'hico_20160224_det/images/test2015/*.jpg'):
        ...
        im_detect(sess, net, im_file, Test_RCNN, keys, scores_P, None, scores_L, bboxes, hdet, None)
