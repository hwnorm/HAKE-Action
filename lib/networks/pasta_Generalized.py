...
class ResNet50(): 
    def __init__(self):
        ...
        self.gt_class_P0 = tf.placeholder(tf.float32, shape=[None, 16], name = 'gt_class_p1s') # target ankle status
        self.gt_class_P1 = tf.placeholder(tf.float32, shape=[None, 15], name = 'gt_class_p2s') # target knee status
        self.gt_class_P2 = tf.placeholder(tf.float32, shape=[None, 6], name = 'gt_class_p3s') # target hip status
        self.gt_class_P3 = tf.placeholder(tf.float32, shape=[None, 34], name = 'gt_class_p4s') # target hand status
        self.gt_class_P4 = tf.placeholder(tf.float32, shape=[None, 8], name = 'gt_class_p5s') # target shoulder status
        self.gt_class_P5 = tf.placeholder(tf.float32, shape=[None, 14], name = 'gt_class_p6s') # target head status
        self.gt_verb     = tf.placeholder(tf.float32, shape=[None, 156], name = 'gt_class_verb') # target verb
        ...
        self.pasta0_weight   = np.ones((1, 16), dtype = 'float32')#np.array([ 0.061241616 , 0.33891392 , 0.92476034 , 5.8682323 , 23.186672 , 17.28461 , 3.0083976 , 16.112774 , 21.125637 , 0.4825653 , 11.593336 , 0.0128623145], dtype='float32').reshape(1, -1)
        self.pasta1_weight   = np.ones((1, 15), dtype = 'float32')#np.array([ 1.1284876 , 7.0736904 , 28.294762 , 21.092459 , 0.58887583 , 1.6362275 , 0.24495044 , 25.779675 , 14.147381 , 0.013494855], dtype='float32').reshape(1, -1)
        self.pasta2_weight   = np.ones((1, 6), dtype = 'float32')#np.array([ 2.3774152 , 9.6559 , 9.986509 , 77.28022 , 0.6999586], dtype='float32').reshape(1, -1)
        self.pasta3_weight   = np.ones((1, 34), dtype = 'float32')#np.array([ 0.003327214 , 0.43338323 , 0.064010605 , 0.05624761 , 0.23240773 , 1.5325437 , 0.3501507 , 0.6822291 , 0.60773283 , 6.6090946 , 1.6785003 , 15.106503 , 10.574552 , 0.15573713 , 1.7335329 , 0.80110234 , 1.0266554 , 0.40360883 , 0.15997808 , 0.19618833 , 0.3659014 , 0.3596786 , 0.39165002 , 0.15573713 , 0.51084787 , 52.872757 , 0.175657 , 0.2360391 , 1.5782912 , 0.9441564 , 0.0018035152], dtype='float32').reshape(1, -1)
        self.pasta4_weight   = np.ones((1, 8), dtype = 'float32')#np.array([ 13.624015 , 24.18162 , 32.617817 , 29.30141 , 0.27514917], dtype='float32').reshape(1, -1)
        self.pasta5_weight   = np.ones((1, 14), dtype = 'float32')#np.array([ 0.08787887 , 0.009932528 , 94.55767 , 1.5009155 , 0.09127188 , 0.8081853 , 0.06847043 , 0.8081853 , 0.28653836 , 0.2781108 , 1.390554 , 0.11111359 , 0.0011817936], dtype='float32').reshape(1, -1)
    ...
    def build_network(self, is_training):
        ...
        fc5_H = self.res5(pool5_H, None, is_training, name='res5_HR')
        ...
        fc5_P0, fc5_P1, fc5_P2, fc5_P3, fc5_P4, fc5_P5 = self.ROI_for_parts(head, fc5_H, None, fc5_S, self.P_boxes, 'ROI_for_parts')
    ...
    def test_image_H(self, sess, image, blobs):
        feed_dict = {self.image: image, 
                     self.H_boxes: blobs['H_boxes'], self.P_boxes: blobs['P_boxes'],
                     self.H_num: blobs['H_num']}
        predictions = sess.run([self.predictions["cls_prob_P"], self.predictions["cls_score_L"]], feed_dict=feed_dict)
        return predictions
